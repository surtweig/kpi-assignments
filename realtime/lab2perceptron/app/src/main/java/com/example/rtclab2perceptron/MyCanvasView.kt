...
class MyCanvasView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    public fun AddPoint(X: Float, Y: Float, C: Int) {
    ...
    private fun drawPoints() {
        for (p in points.iterator()) {
            if (p.color == 0)
                extraCanvas.drawCircle(p.posX, p.posY, pointRadius, pointNegPaint)
            else
                extraCanvas.drawCircle(p.posX, p.posY, pointRadius, pointPosPaint)
        }
    }

    private fun pickPoint(X: Float, Y: Float): Point? {
        val pr2 = pointRadius*pointRadius
        for (p in points.iterator()) {
            val dX = p.posX - X
            val dY = p.posY - Y
    ...
    private fun touchStart() {
        ...
        selectedPoint = pickPoint(currentX, currentY)
    }
    ...
    private fun touchMove() {
        ...
        if (selectedPoint != null) {
            selectedPoint?.posX = currentX
            selectedPoint?.posY = currentY
        }
        extraCanvas.drawColor(backgroundColor)
        drawPoints()
